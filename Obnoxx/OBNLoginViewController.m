...
@property (nonatomic, strong) IBOutlet UIButton *verfiy;
...
-(IBAction) verify:(id) sender
{
    ...
    [server addObserver:self forKeyPath:@"verifyResponse" options:NSKeyValueChangeNewKey context:@"lvc1"];
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(NSObject *)response change:(NSString *)change context:(id) context
{
    ...
    if([context isEqualToString:@"lvc1"])
    {
        ...
        if(success)
        {
            ...
            appState.temporaryToken = [server.verifyResponse valueForKey:@"temporaryUserCode"];
